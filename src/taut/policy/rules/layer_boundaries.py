...
def layer_boundary_rule_definitions() -> tuple[RuleDefinition, ...]:
    ...
    definitions = (
        (
            ENTRY_RULE_ID,
            "No direct DB or external calls in entrypoints",
            "Routers, Consumers, and Tasks must use Services for DB and external operations.",
            _RoleBoundaryRule(ENTRY_RULE_ID, "entry_roles", "entry"),
            "entry_boundary",
        ),
        (
            SERVICE_RULE_ID,
            "No DB primitives or transport logic in Services",
            "Keep transactions in Services, SQL in Models/Queries, and HTTP logic in Routers.",
            _RoleBoundaryRule(SERVICE_RULE_ID, "service_roles", "service"),
            "service_boundary",
        ),
        (
            QUERY_RULE_ID,
            "Read-only Queries boundary",
            "Do not open sessions, mutate data, or call external systems in Queries.",
            _RoleBoundaryRule(QUERY_RULE_ID, "query_roles", "query"),
            "query_boundary",
        ),
        (
            MODEL_RULE_ID,
            "No external connections in Models",
            "Keep Models focused on persistence, without external APIs or HTTP representation.",
    ...
    dependency = RuleDefinition(
        ...
        "Depends placement",
        "Use request dependency injection only in entrypoints such as Routers and Consumers.",
        RuleTarget.MODULE,
        requirements,
        ChangeImpact.SELF,
